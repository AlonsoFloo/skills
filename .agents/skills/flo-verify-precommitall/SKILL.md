...
# Verify Pre-Commit All

Verify pre-commit installation, execute hooks across all files with minimal output, auto-fix formatting/linter issues, and output a concise summary.
...
- **Linter / Schema / Gitleaks errors:** If any hook fails requiring manual intervention:
  ...
  - Re-run `pre-commit run --all-files` (or `pre-commit run <hook_id> --all-files`) until all checks pass with exit code 0.
