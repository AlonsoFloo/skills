...
1. **GitHub Workflows (`.github/workflows/pr-checks.yml` & `.github/workflows/release-please.yml`)**
1. **GitHub Templates (`.github/ISSUE_TEMPLATE/` & `.github/PULL_REQUEST_TEMPLATE.md`)**
1. **Gitignore (`.gitignore`)**
1. **Gitconfig / Git Attributes (`.gitattributes` & `.gitconfig`)**
...
1. Verify pre-commit installation and run hooks across all files:
   ```bash
   pre-commit run --all-files
   ```
1. Auto-fix formatting or hook failures if needed.
